...
const ServiceName = "keptn-django-migrate-service"
...
type MigrateTriggeredEventData struct {
	keptnv2.EventData
}
...
func processKeptnCloudEvent(ctx context.Context, event cloudevents.Event) error {
	...
	**/
	...
	case keptnv2.GetTriggeredEventType("migrate"): // sh.keptn.event.migrate.triggered
		log.Printf("Processing migrate.triggered Event")

		eventData := &MigrateTriggeredEventData{}
		parseKeptnCloudEventPayload(event, eventData)

		return HandleMigrateTriggeredEvent(myKeptn, event, eventData)
	}
...
func _main(args []string, env envConfig) int {
	...
	log.Println("Starting keptn-django-migrate-service...")
